    ...
    for obj in objects:
      if obj.score>0.7:
        ...
        for idx in keep:
            ...
            int(boxes[idx][2]), int(boxes[idx][3]),scores[idx]])   
...
def compute_ap(gt_boxes, pred_boxes, iou_thresh=0.5):
    ...
    for i in range(0, len(precision)):
       ...
       if i==len(precision)-1 and len(precision)>1:        #计算最后一个recall的面积
          ap+=(1-recall[i])*precision[i]
...
if __name__ == '__main__':
    ...
    for i in range(x):
        ...
        AP = eval1_instance.evalution()     # 调用评估方法
        if AP==0:
          flag+=1
          continue
        else:
    ...
    final_map = MAP / (x-flag)       #去除不含汽车类别的点
    print("#########################")
    print(f"Car 类别的平均 AP（mAP）为: {final_map:.4f}")
    print("评估完成。")
