...
class kitti_object(object):
    ...
    def __init__(self, root_dir, split="training", args=None):
        ...
        self.detect_dir = os.path.join(self.split_dir, "detect")
        self.seg_dir = os.path.join(self.split_dir, "segment")	
        self.calib_dir = os.path.join(self.split_dir, "calib")
        self.detect3D =os.path.join(self.split_dir, "detect3D")
        self.FPN =os.path.join(self.split_dir, "FPN50")
        self.point =os.path.join(self.split_dir, "point")
        self.point100 =os.path.join(self.split_dir, "point100")
    ...
    def get_seg(self, idx):                                                ##################################################
        assert idx < self.num_samples
        img_filename= os.path.join(self.seg_dir, "%06d.png" % (idx))
        return utils.load_image(img_filename)
    ...
    def get_detects(self, idx):                                                ##################################################
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.detect_dir, "%06d.txt" % (idx))
        return utils.read_label(label_filename)
    def get_FPN(self, idx):                                                ##################################################
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.FPN, "%06d.txt" % (idx))
        return utils.read_label(label_filename)
    def get_point(self, idx):                                                ##################################################
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.point, "%06d.txt" % (idx))
        return utils.read_label(label_filename)
    def get_point100(self, idx):                                                ##################################################
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.point100, "%06d.txt" % (idx))
        return utils.read_label(label_filename)        
    def get_detects3D(self, idx):                                                ##################################################
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.detect3D, "%06d.txt" % (idx))
        return utils.read_label(label_filename)
...
def show_image_with_boxes(img, objects, calib, show3d=True, depth=None):                    # 3. 图片上绘制3D bbox
...
def nms(boxes, scores, iou_threshold=0.5):
    ...
    boxes = torch.tensor(boxes, dtype=torch.float32)  # 将 x1 转换为张量
    #boxes = np.array(boxes)
    #scores = np.array(scores)
    scores = torch.tensor(scores, dtype=torch.float32)  # 将 x1 转换为张量
    ...
    while len(sorted_indices) > 0:
        ...
        # 计算 IoU
        xx1 = torch.maximum(x1[current_idx], x1[sorted_indices[1:]])
        yy1 = torch.maximum(y1[current_idx], y1[sorted_indices[1:]])
        xx2 = torch.minimum(x2[current_idx], x2[sorted_indices[1:]])
        yy2 = torch.minimum(y2[current_idx], y2[sorted_indices[1:]])
        
        inter = torch.maximum(xx2 - xx1 + 1, torch.tensor(0)) * torch.maximum(yy2 - yy1 + 1, torch.tensor(0))
        iou = inter / (areas[current_idx] + areas[sorted_indices[1:]] - inter)
        
        # 仅保留 IoU 低于阈值的框
        sorted_indices = sorted_indices[1:][iou < iou_threshold]
    return torch.tensor(keep)
def fusion33D(pc_velo, img, calib, img_width, img_height,objects,detects):         
        ...
        if obj.score > 0.7:
            ...
            if obj.type == "Car":
                ...
                for i in range(imgfov_pts_2d.shape[0]):
                    ...
                    if obj.xmin < x < obj.xmax and obj.ymin < y < obj.ymax:
                        car.append(i)
    ...
    return img
def fusion333D(pc_velo, calib, objects, detects):  #6.0 视锥体生成
    """ Project LiDAR points to image """
    fig = mlab.figure(
    ...
    for obj in detects:
        ...
        if obj.score > 0.7:
            ...
            if obj.type == "Car":
                if True:
                    x = get_lidar_in_image_fov(
                        x[:, 0:3], calib, obj.xmin, obj.ymin, obj.xmax, obj.ymax
                    )
                draw_lidar(x, fig=fig)
                color = (0, 1, 0)
    mlab.show(1)
def fusion3D1(pc_velo, calib, objects, detects):  #6.1 聚类方法！
    """ Project LiDAR points to image """
    fig = mlab.figure(
        figure=None, bgcolor=(0, 0, 0), fgcolor=None, engine=None, size=(1000, 500)
    )
    for obj in detects:
        if obj.score > 0.7 and obj.type == "Car":
            # 提取视锥体内的点云（根据图像检测框）
            pc_sub = get_lidar_in_image_fov(
                pc_velo[:, 0:3], calib, obj.xmin, obj.ymin, obj.xmax, obj.ymax
            )
            draw_lidar(pc_sub, fig=fig)
            
            color = (0, 1, 0)
            # 如果点太少就跳过
            if pc_sub.shape[0] < 20:
                continue
            # DBSCAN 聚类
            clustering = DBSCAN(eps=0.8, min_samples=60).fit(pc_sub)
            labels = clustering.labels_
            unique_labels = set(labels)
            for label in unique_labels:
                if label == -1:
                    continue  # -1 是噪声
                cluster = pc_sub[labels == label]
                color = (0.0, 0.0, 1.0)  # 蓝色
                # 画聚类点
                mlab.points3d(
                    cluster[:, 0], cluster[:, 1], cluster[:, 2],
                    mode="point", color=color, figure=fig
                )
                # 可视化包围框
                ...
                for i, j in edges:
                    mlab.plot3d(
                        ...
                        color=color, line_width=1.0, figure=fig
                    )
    mlab.show(1)

def fusion3D2(pc_velo, calib, objects, detects):
    """6.3-1 基于点云文件的融合方法：Project LiDAR points to image + 2D检测框聚类融合"""
    ...
    for obj in detects:
        if obj.score > 0.7 and obj.type == "Car":
            ...
            # 如果点太少就跳过
            if pc_sub.shape[0] < 10:
                continue
            # DBSCAN 聚类
            clustering = DBSCAN(eps=0.8, min_samples=100).fit(pc_sub)
            labels = clustering.labels_
            unique_labels = set(labels)
    ...
    mlab.show(1)
import mmcv.ops
def compute_box_3d_from_param(x, y, z, h, w, l, ry):
    """
    计算 3D 边界框的 8 个角点，输入为 KITTI 格式参数。
    返回值为 (8, 3) 的 ndarray，表示 8 个角点的坐标。
    """
    # 3D 边界框的局部坐标（以中心为原点）
...
def fusion3D3(pc_velo, calib, objects, detects):           #6.3-2 基于点云文件的融合方法！ NMS之后
    ...
    for obj in detects:
        if obj.score > 0.7 and obj.type == "Car":
            ...
            for label in unique_labels:
                ...
                pca.fit(cluster)
                main_axis = pca.components_[0]  # shape: (3,)
                center = cluster.mean(axis=0)
                yaw = np.arctan2(main_axis[1], main_axis[0])  # 输出就是绕z轴旋转角，单位：弧度
                x_min, y_min, z_min = cluster.min(axis=0)
                x_max, y_max, z_max = cluster.max(axis=0)
                
                draw.append(np.array([
                    [x_min, y_min, z_min],
                    [x_max, y_min, z_min],
                    [x_max, y_max, z_min],
                    [x_min, y_max, z_min],
                 ...
                 #计算聚类框的长宽、位置
                center = (cluster.min(axis=0) + cluster.max(axis=0)) / 2
                dx, dy, dz = cluster.max(axis=0) - cluster.min(axis=0)
                heading = 0.0
                bo = [center[0], center[1], center[2], dx, dy, dz, heading]
                box.append(bo)
    ...
    for obj in objects:
      if obj.score >0.7:
        ...
        if obj.type == "Car":
           #计算检测框的方向位置、角度
            center_cam = np.array([obj.t[0], obj.t[1], obj.t[2]]).reshape(1, 3)  # 相机系下的中心点
            center_velo = calib.project_rect_to_velo(center_cam)[0]  # 转为雷达系
            box.append([center_velo[0],center_velo[1],center_velo[2],obj.l,obj.h,obj.w,obj.ry])
            score.append(obj.score)
    if len(box) == 0:
        print("无有效框，直接退出")
        return
    # ---------- NMS ----------
    boxes_tensor = torch.tensor(box, dtype=torch.float32).cuda()
    scores_tensor = torch.tensor(score, dtype=torch.float32).cuda()
    keep_indices = mmcv.ops.nms3d(boxes_tensor, scores_tensor,0.5)            #-————————————————————NMS设置
    print(keep_indices)
    ...
    for idx in keep_indices:
      if idx<len(draw):
        for i, j in edges:
                    mlab.plot3d(
                        [draw[idx][i][0], draw[idx][j][0]],
                        [draw[idx][i][1], draw[idx][j][1]],
                        [draw[idx][i][2], draw[idx][j][2]],
                        color=(1, 1, 0), line_width=1.0, figure=fig
                    )
      else:
        bo = box[idx]
        corners_3d = compute_box_3d_from_param(*bo)  # 转换成8个点
        draw_gt_boxes3d([corners_3d], fig=fig, color=(0, 1, 0))
    mlab.show(1)
from sklearn.decomposition import PCA
def fusion3D(pc_velo, img, calib, img_width, img_height, objects, detects):              
    """ Project LiDAR points to image and fit 3D bounding boxes with clustering """
    img = np.copy(img)
    ...
    for obj in detects:
        if obj.score > 0.7:
            if obj.type in ["Car"]:
                ...
                # 提取属于该2D框内的点索引
                car_indices = []
                for i in range(imgfov_pc_rect.shape[0]):
                    x = int(np.round(imgfov_pts_2d[i, 0]))
                    y = int(np.round(imgfov_pts_2d[i, 1]))
                    if obj.xmin < x < obj.xmax and obj.ymin < y < obj.ymax:
                        depth = imgfov_pc_rect[i, 2]
                        xy_depth = np.array([x, y, depth])
                        car_indices.append(xy_depth)
                        color = cmap[int(640.0 / depth), :]
                        cv2.circle(
                            img,
                            (x, y),
                ...
                if len(car_indices) > 0:
                    clustering = DBSCAN(eps=80, min_samples=3).fit(car_indices)
                    labels = clustering.labels_
                    ...
                    if len(unique_labels) > 0:
                        ...
                        car_cluster_pts = car_indices[labels == largest_label]

                        pcd = o3d.geometry.PointCloud()
                        pcd.points = o3d.utility.Vector3dVector(car_cluster_pts)
                        points = np.asarray(pcd.points)
                        
                        xz_points = points[:, [0, 2]]  # 拟合 Y轴方向的旋转（即绕 X 轴）
                        yz_points = points[:, [0, 1]]  # 拟合 X轴方向的旋转（即绕 Y 轴）
                        
                        pca_xz = PCA(n_components=2).fit(xz_points)
                        angle_y = np.arctan2(pca_xz.components_[0][1], pca_xz.components_[0][0])  # 绕 Y 轴旋转
                        
                        pca_yz = PCA(n_components=2).fit(yz_points)
                        angle_x = -np.arctan2(pca_yz.components_[0][1], pca_yz.components_[0][0])  # 绕 X 轴旋转
                        R_x = np.array([
    [1, 0, 0],
    ...
    [-np.sin(angle_y), 0, np.cos(angle_y)]
                        ...
                        pcd_rotated = pcd.rotate(R_total, center=center)
                        aabb = pcd_rotated.get_oriented_bounding_box()
 

                        print("3D 包围框角点：", np.asarray(aabb.get_box_points()))
                        
                        obb=np.asarray(aabb.get_box_points())
                        points_2d = obb[:, :2].astype(np.int32)
                        edges = [
    (0, 1), (0, 2), (0, 3),
    (1, 6), (1, 7),
    (2, 5), (2, 7),
...
def fusion_2D(img, objects,detects, calib, depth=None):              ########################################################################
    """ Show image with 2D bounding boxes """
    img1 = np.copy(img)  # for 2d bbox
    img2 = np.copy(img)  # 没有非极大值抑制的融合
    #img3 = np.copy(img)  # for 3d bbox
    #TODO: change the color of boxes
    boxes=[]
    scores=[]
    ...
    for obj in detects: 
      if obj.score>0.7:
        ...
        if obj.type == "DontCare":
            continue
        if obj.type == "Car":
            boxes.append([int(obj.xmin),int(obj.ymin),int(obj.xmax),int(obj.ymax)])
...
def get_lidar_in_image_fov(                                                                    #1 得到图像范围内的点云
...
def show_lidar_with_boxes(                              #0 在雷达文件上生成3D检测框体
    pc_velo,
    objects,
    calib,
    img_fov=False,
    ...
    for obj in objects:
        if obj.type == "DontCare":
            continue
        if hasattr(obj, 'score') and obj.score is not None and obj.score < 0.5:	
            continue
        # Draw 3d bounding box
        _, box3d_pts_3d = utils.compute_box_3d(obj, calib.P)
        box3d_pts_3d_velo = calib.project_rect_to_velo(box3d_pts_3d)
...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height):
    ...
    # 设置最大深度，用来归一化
    max_depth = 80.0  # 单位米，可以根据你的点云距离调整

    for i in range(imgfov_pts_2d.shape[0]):
        depth = imgfov_pc_rect[i, 2]

        if depth <= 0:  # 防止除0或负数情况
            continue

        # 归一化深度到 0-255
        depth_norm = np.clip(depth / max_depth, 0, 1)
        color_idx = int(depth_norm * 255)
